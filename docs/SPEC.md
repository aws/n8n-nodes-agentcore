...
`iam:PassRole` is not required by the harness API itself: the execution role ARN is passed to CreateHarness as a parameter and assumed by the AgentCore service, not passed by the caller. Note that CreateHarness provisions several underlying resources (AgentCore Runtime, runtime endpoint, memory store, workload identity) and IAM authorizes each against its own resource type, so a first run in a fresh account may surface additional caller grants — `iam:PassRole` on the execution role, `CreateAgentRuntime`, `CreateAgentRuntimeEndpoint`, `iam:CreateServiceLinkedRole`, and `CreateWorkloadIdentity` have all been reported. The AWS harness security guide is the source of truth.
...
Base permissions (the AWS guide's sample policy, public-network harness): `bedrock:InvokeModel(WithResponseStream)`, `ecr-public:GetAuthorizationToken` + `sts:GetServiceBearerToken` (managed container pull from ECR Public), `xray:*` trace APIs, `logs:*` scoped to `/aws/bedrock-agentcore/runtimes/*`, `cloudwatch:PutMetricData` (namespace-conditioned), `GetWorkloadAccessToken(ForJWT)`, and the built-in Browser / Code Interpreter session actions scoped to `…:<region>:aws:browser/*` and `…:code-interpreter/*`.

**VPC mode changes the container pull.** A VPC-mode harness pulls the managed image from a **private** ECR repository in the harness Region (`repository/harness-*`), not ECR Public, so its execution role additionally needs `ecr:BatchGetImage`, `ecr:GetDownloadUrlForLayer`, `ecr:BatchCheckLayerAvailability` (resource `arn:aws:ecr:<region>:*:repository/harness-*`, account wildcarded because the repo is AWS-owned) and `ecr:GetAuthorizationToken`. The subnets need no internet egress; instead the VPC needs interface endpoints for `ecr.dkr` and `ecr.api`, a gateway endpoint for `s3`, and an interface endpoint for `bedrock-runtime` when the agent calls Bedrock. Canonical list: [Network configuration](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/harness-security.html#harness-network-config).
